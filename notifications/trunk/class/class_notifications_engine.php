...
class notifications extends request
{
	...
	public function add_flag_for_ressources($ressources, $column_flag, $verif='false')
	//Tag document when it's over, switch flag_notif
	{
		...
		if ($_SESSION['debug']['tag_when_send'] == "true" )
		{
			if($verif == 'true')
			{
				...
				{
						$this->query("UPDATE ".$_SESSION['collection'][0]['extensions'][0]." set ".$column_flag." = 'Y' where RES_ID= '".$ressources[$l]."'");
				}
			}
			else
			{
				echo "notice : unable to updating flags beacause mail don't send\r\n";
			}
		}
		else
		{
