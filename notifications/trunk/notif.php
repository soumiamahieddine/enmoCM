...
else
{
        ...
        for($ui=0; $ui < count($users); $ui++)
        {
            ...
            $list_trait = $notification->get_ressource_for_process($users[$ui]);
            if ($list_trait <> false)
            {
            ...
            {
                $list_copy = $notification->get_ressource_for_copy($users[$ui]);
        ...
        //$notification->add_flag_for_ressources($flag_for_notif, 'flag_notif', $verif);
        $notification->add_flag_for_ressources($resources, 'flag_notif');
}
