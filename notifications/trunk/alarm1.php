...
else
{
        ...
        $template_path = $_SESSION['config']['MaarchDirectory']."modules".DIRECTORY_SEPARATOR."notifications".DIRECTORY_SEPARATOR.$_SESSION['templates_directory'].DIRECTORY_SEPARATOR.$_SESSION['templates']['alarm1'];
        $template_path_copy = $_SESSION['config']['MaarchDirectory']."modules".DIRECTORY_SEPARATOR."notifications".DIRECTORY_SEPARATOR.$_SESSION['templates_directory'].DIRECTORY_SEPARATOR.$_SESSION['templates']['alarm1_copy'];
        ...
        for($ui=0; $ui < count($users); $ui++)
        {
            ...
            $list_trait = $notification->get_ressource_for_alarm1($users[$ui]);
            if ($list_trait <> false)
            {
                ...
                foreach ($list_trait as $this_list_trait) {
                    array_push ($resources, $notification->get_doc_info($this_list_trait, $verif));
                }
            }
            if ($_SESSION['features']['copy_for_alarm1'] == 'true')
            //sending copy for this new ressources only if features copy_for_notif is enabled
            {
                $list_copy = $notification->get_ressource_for_alarm1_copy($users[$ui]);
        ...
        //$notification->add_flag_for_ressources($flag_for_notif, 'flag_alarm1', $verif);
        $notification->add_flag_for_ressources($resources, 'flag_alarm1');
}
