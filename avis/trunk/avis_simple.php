...
require_once "modules" . DIRECTORY_SEPARATOR . "avis" . DIRECTORY_SEPARATOR
 ...
 function get_form_txt($values, $path_manage_action,  $id_action, $table, $module, $coll_id, $mode )
 {
    ...
    $templates = array();
        ...
        $frm_str .= '<textarea style="width:98%;height:60px;resize:none;" rows="10" name="notes"  id="notes" onblur="document.getElementById(\'note_content_to_users\').value=document.getElementById(\'notes\').value.replace(/[\n]/gi, \'##\' );"></textarea>';
...
function manage_form($arr_id, $history, $id_action, $label_action, $status, $coll_id, $table, $values_form )
{
    ...
    */
    ...
    $stmt = $db->query("UPDATE listinstance SET process_date = CURRENT_TIMESTAMP "
            . " WHERE (item_mode = ? or item_mode = ?) or AND res_id = ? AND item_id = ? AND difflist_type = ?"
