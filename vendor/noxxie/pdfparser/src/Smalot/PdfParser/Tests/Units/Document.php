...
class Document extends atoum\test
{
    public function testSetObjects()
    {
        ...
        // Obj #1 exists
        $this->assert->object($document->getObjectById(1))->isInstanceOf('\Noxxie\PdfParser\PDFObject');

        $content = '<</Type/Page>>';
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object  = new \Noxxie\PdfParser\PDFObject($document, $header);
        $document->setObjects(array(2 => $object));
        // Obj #1 is missing
        $this->assert->assert->variable($document->getObjectById(1))->isNull();
        // Obj #2 exists
        $this->assert->object($document->getObjectById(2))->isInstanceOf('\Noxxie\PdfParser\PDFObject');
    }

    public function testGetObjects()
    {
        $document = new \Noxxie\PdfParser\Document();
        $object1  = new \Noxxie\PdfParser\PDFObject($document);
        $content  = '<</Type/Page>>unparsed content';
        $header   = \Noxxie\PdfParser\Header::parse($content, $document);

        $object2 = new \Noxxie\PdfParser\Page($document, $header);
        $document->setObjects(array(1 => $object1, 2 => $object2));

        $this->assert->integer(count($objects = $document->getObjects()))->isEqualTo(2);
        $this->assert->object($objects[1])->isInstanceOf('\Noxxie\PdfParser\PDFObject');
        $this->assert->object($objects[2])->isInstanceOf('\Noxxie\PdfParser\PDFObject');
        $this->assert->object($objects[2])->isInstanceOf('\Noxxie\PdfParser\Page');
    }
    ...
    public function testGetPages()
    {
        ...
        $content = '<</Type/Page>>';
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object1 = new \Noxxie\PdfParser\Page($document, $header);
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object2 = new \Noxxie\PdfParser\Page($document, $header);
        $document->setObjects(array(1 => $object1, 2 => $object2));
        $pages = $document->getPages();
        $this->assert->integer(count($pages))->isEqualTo(2);
        $this->assert->object($pages[0])->isInstanceOf('\Noxxie\PdfParser\Page');
        $this->assert->object($pages[1])->isInstanceOf('\Noxxie\PdfParser\Page');
        ...
        $object2 = new \Noxxie\PdfParser\Page($document, $header);
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object3 = new \Noxxie\PdfParser\Page($document, $header);
        $content = '<</Type/Pages/Kids[1 0 R 2 0 R]>>';
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object4 = new \Noxxie\PdfParser\Pages($document, $header);
        $content = '<</Type/Pages/Kids[3 0 R]>>';
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object5 = new \Noxxie\PdfParser\Pages($document, $header);
        $document->setObjects(
        ...
        $this->assert->object($pages[0])->isInstanceOf('\Noxxie\PdfParser\Page');
        $this->assert->object($pages[1])->isInstanceOf('\Noxxie\PdfParser\Page');
        $this->assert->object($pages[2])->isInstanceOf('\Noxxie\PdfParser\Page');
        ...
        $header  = \Noxxie\PdfParser\Header::parse($content, $document);
        $object6 = new \Noxxie\PdfParser\Pages($document, $header);
        $document->setObjects(
            array(
                '1_0' => $object1,
                '2_0' => $object2,
