...
function validate_cs_submit($mode)
{
    ...
    if (isset($_REQUEST['id'])) {
        $docserver_types->docserver_type_id = $_REQUEST['id'];
    }
    if (isset($_REQUEST['docserver_type_label'])) {
        $docserver_types->docserver_type_label = $_REQUEST['docserver_type_label'];
    }
    if (isset($_REQUEST['is_container'])) {
        $docserver_types->is_container = $_REQUEST['is_container'];
    }
    if (isset($_REQUEST['container_max_number'])) {
        $docserver_types->container_max_number = $_REQUEST['container_max_number'];
    }
    if (isset($_REQUEST['is_compressed'])) {
        $docserver_types->is_compressed = $_REQUEST['is_compressed'];
    }
    if (isset($_REQUEST['compression_mode'])) {
        $docserver_types->compression_mode = $_REQUEST['compression_mode'];
    }
    if (isset($_REQUEST['is_meta'])) {
        $docserver_types->is_meta = $_REQUEST['is_meta'];
    }
    if (isset($_REQUEST['meta_template'])) {
        $docserver_types->meta_template = $_REQUEST['meta_template'];
    }
    if (isset($_REQUEST['is_logged'])) {
        $docserver_types->is_logged = $_REQUEST['is_logged'];
    }
    if (isset($_REQUEST['log_template'])) {
        $docserver_types->log_template = $_REQUEST['log_template'];
    }
    if (isset($_REQUEST['is_signed'])) {
        $docserver_types->is_signed = $_REQUEST['is_signed'];
    }
    if (isset($_REQUEST['fingerprint_mode'])) {
        $docserver_types->fingerprint_mode = $_REQUEST['fingerprint_mode'];
    }
    $control = array();
            ...
            switch ($item['column']) {
                ...
                case 'is_container':
                    if ($item['value'] == 'Y') {
                        ...
                        $item['value'] = "<i class='fa fa-times fa-2x' title='"._NOT_CONTAINER."'></i>";
                    }
                ...
                case 'is_compressed':
                    if ($item['value'] == 'Y') {
                        ...
                        $item['value'] = "<i class='fa fa-times fa-2x' title='"._NOT_COMPRESSED."'></i>";
                    }
...
function display_del($docserver_type_id)
{
    ...
    if (isset($docserver_types)) {
        ...
        $pageName = 'docserver_types_management_controler'; ?><script>window.top.location='<?php echo $_SESSION['config']['businessappurl'].'index.php?page='.$pageName.'&mode=list&admin=docservers'; ?>';</script>
		<?php
        exit;
    } else {
        // Error management
