    ...
    for ($j=0;$j<count($tab[$i]);$j++)
    {
        foreach(array_keys($tab[$i][$j]) as $value)
        {
            ...
            if($tab[$i][$j][$value]=="viewed")
            {
                ...
                $stmt = $db->query("SELECT viewed FROM listinstance WHERE res_id = ? and coll_id = ?"
                    . " and item_id = ?",
...
$paramsTab['filters'] = array('entity', 'entity_subentities', 'category', 'isViewed');                     //Filtres    
...
$content .= "<script>$j('#container').attr('style', 'width: 90%; min-width: 1000px;');".
