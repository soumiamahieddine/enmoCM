...
if (isset($whereRequest) && !empty($whereRequest)) {
    ...
    if ($view == 'res_view_attachments') {
        echo '<ul id="list_ans" title="Liste des réponses">';
    } else {
        echo '<ul id ="list">';
    } ?>
            ...
            for ($j = 0; $j < count($tab[$i]); ++$j) {
                foreach (array_keys($tab[$i][$j]) as $value) {
                    ...
                    if ($tab[$i][$j][$value] == 'attachment_type') {
                        ...
                        if ($tab[$i][$j]['value'] == 'signed_response') {
                            $line .= ' <i class="fa fa-certificate fa-1x"></i>';
                        }
                        $line .= '</span><br/><br/>';
                    }
                    ...
                    if ($tab[$i][$j][$value] == 'creation_date') {
                        $line .= '<span style="font-style:italic;font-size:12px;"><i class="fa fa-calendar-alt fa-2x"></i> '.functions::format_date_db(
                            $tab[$i][$j]['value'], false
                        );
                    }
