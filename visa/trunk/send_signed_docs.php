...
require_once('core' . DIRECTORY_SEPARATOR . 'class' . DIRECTORY_SEPARATOR 
 ...
 function checkAllSigned($id){
	$db_check = new dbquery();
	$db_check->connect();
	$db_check->query("SELECT status from res_view_attachments where attachment_type='response_project' and res_id_master = ".$id);
	while($line = $db_check->fetch_object()){
