...
abstract class visa_Abstract extends Database
{
	...
	*/
	...
	public function checkResponseProject($res_id, $coll_id) {
		...
		$db = new Database();
		$stmt = $db->query("SELECT * FROM res_attachments WHERE res_id_master = ? AND coll_id = ? AND status NOT IN ('DEL','OBS') AND attachment_type IN ('response_project', 'print_folder', 'signed_response','outgoing_mail','waybill','transfer', 'simple_attachment', 'envelope', 'transmission', 'aihp') ", array($res_id, $coll_id));
		if ($stmt->rowCount() <= 0) {
