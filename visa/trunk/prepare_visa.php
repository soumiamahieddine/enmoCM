...
* pour le reste des opérations. Ce circuit est stockée dans la BDD (table visa_circuit)
*
* @file
...
function get_rep_path($res_id, $coll_id)
{
	...
	$db->query("select filename, path,title,res_id,relation  from res_view_attachments where res_id_master = " . $res_id . " AND status <> 'OBS' AND status <> 'DEL' and attachment_type = 'response_project' order by creation_date asc");
	...
	while ($res2 = $db->fetch_object()){
		...
		$array_reponses[$cpt_rep]['relation'] = $res2->relation;
		$cpt_rep++;
		}
	}
...
function get_form_txt($values, $path_manage_action,  $id_action, $table, $module, $coll_id, $mode )
{
	...
	$frm_str .= '<h1 class="tit" id="action_title"><img src="'.$_SESSION['config']['businessappurl'].'static.php?filename=logo_action_18.png"  align="middle" alt="" />'._PREPARE_VISA.' '._NUM.$resChrono->alt_identifier;
	$frm_str .= '</h1>';
	...
	$frm_str .= '<dt id="onglet_details">'. _DETAILS .'</dt><dd id="page_details">';
	...
	foreach($data as $key=>$d){
		if ($key != 'folder' && $key != 'confidentiality'){
			if ($cpt_i%2 == 0){
				$frm_str .= '<tr class="col">';
			}
			$frm_str .= '<th class="picto" align="left"><img src="'.$data[$key]['img'].'" title="'.$data[$key]['label'].'" /></th>';
	...
	if ($core->is_module_loaded('attachments'))
	{
		...
		$countAttachments = "select res_id, creation_date, title, format from res_view_attachments where res_id_master = " . $_SESSION['doc_id'] 
				. " and coll_id ='" . $_SESSION['collection_id_choice'] 
				. "' and status <> 'DEL' and status <> 'OBS' and attachment_type='response_project' ";
	...
	if ($core->test_service('sendmail', 'sendmail', false) === true) {
		...
		$frm_str .= '<dt id="onglet_courriels">' . _SENDED_EMAILS.$nbr_emails .'</dt><dd id="page_courriels">';
		//Emails iframe
	...
	$frm_str .= $visa_circuit->getList($res_id, $coll_id, true);*/
	...
	for ($i=0; $i<count($tab_path_rep_file);$i++){
		$num_rep = $i+1;
		$frm_str .= '<dt onclick="updateFunctionModifRep(\''.$tab_path_rep_file[$i]['res_id'].'\', '.$num_rep.',\''.$tab_path_rep_file[$i]['relation'].'\');">'.$tab_path_rep_file[$i]['title'].'</dt><dd>';
	...
	$frm_str .= '<div class="toolsSplit">';	
		...
		if(count($actions) > 0)
		{
			...
			$frm_str .='</select> ';
			$table = $sec->retrieve_table_from_coll($coll_id);
			$frm_str .= '<input type="button" name="send" id="send" value="'._VALIDATE.'" class="button" onclick="if (document.getElementById(\'split_chosen_action\').value == 403 || document.getElementById(\'split_chosen_action\').value == 404 || document.getElementById(\'split_chosen_action\').value == 414) generateBordereau('.$res_id.');valid_action_form( \'index_file\', \''.$path_manage_action.'\', \''. $id_action.'\', \''.$res_id.'\', \''.$table.'\', \''.$module.'\', \''.$coll_id.'\', \''.$mode.'\');"/> ';
		}
		...
		if($data['category_id']['value'] == "incoming") $frm_str .= 'modifyAttachmentsForm(\'' . $_SESSION['config']['businessappurl']
                            . 'index.php?display=true&module=attachments&page=attachments_content&id=' . $tab_path_rep_file[0]['res_id'] . '&relation='.$tab_path_rep_file[0]['relation'].'\');';
	...
	**/
	...
	$frm_str .= '<div id="frm_error_'.$id_action.'" class="indexing_error"></div>';		
	/*$visa_circuit = new visa();
	$frm_str .= $visa_circuit->getList($res_id, $coll_id, true, "tab_visaSetWorkflow_uni");*/
	...
	$frm_str .= '<div class="toolsSplit">';	
		...
		if(count($actions) > 0)
		{
			...
			$frm_str .='</select> ';
			$table = $sec->retrieve_table_from_coll($coll_id);
			$frm_str .= '<input type="button" name="send" id="send" value="'._VALIDATE.'" class="button" onclick="if (document.getElementById(\'uni_chosen_action\').value == 403 || document.getElementById(\'uni_chosen_action\').value == 404 || document.getElementById(\'uni_chosen_action\').value == 414) generateBordereau('.$res_id.');valid_action_form( \'index_file\', \''.$path_manage_action.'\', \''. $id_action.'\', \''.$res_id.'\', \''.$table.'\', \''.$module.'\', \''.$coll_id.'\', \''.$mode.'\');"/> ';
		}
		
		$frm_str .= '</td>';
		
		if($data['category_id']['value'] == "incoming") $frm_str .= '<input type="hidden" name="cur_rep" id="cur_rep" value="'.$tab_path_rep_file[0]['res_id'].'" >';
		$frm_str .= '<input type="hidden" name="cur_idAffich" id="cur_idAffich" value="1" >';
...
function check_form($form_id,$values)
{
    ...
    {
        $type_view = get_value_fields($values, 'typeView');
        $cons1 = get_value_fields($values, $type_view.'_conseiller_1');
		if (!isset($cons1) || $cons1 == "") {
			$_SESSION['action_error'] =  "Il n'y a aucun conseiller sélectionné";
			return false;
		}
        return true;
    }
...
function get_circuit($values, $typeView){
	$tab_circuit = array();
	foreach($values as $key=>$val){
		$vals = explode("_",$val['ID']);
		if (isset($vals[1]) && $vals[1] == "conseiller" && $vals[0] == $typeView){
			array_push($tab_circuit,array('sequence'=>$vals[2], 'vis_user'=>$val['VALUE'], 'note'=>get_value_fields($values, $typeView.'_'.'consigne_'.$vals[2])));
		}
	}	
...
function manage_form($arr_id, $history, $id_action, $label_action, $status,  $coll_id, $table, $values_form )
{
	...
	/*$visa_circuit = new visa();
	$circuit = get_circuit($values_form, $type_view);
	if ($action_chosen == 414){
		$circuit[count($circuit)-2]['note'] .= " - Transmettre pour e-signature";
	}
	elseif ($action_chosen == 403){
		$circuit[count($circuit)-2]['note'] .= " - Transmettre pour impression";
	}
	
	$visa_circuit->saveWorkflow($res_id, $coll_id, $circuit);*/
