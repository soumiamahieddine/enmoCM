		...
		<div id="inner_content" class="clearfix">
			...
			<form name="formbasket" id="formbasket" method="post" action="<?php if($mode == "up") { echo $_SESSION['config']['businessappurl']."index.php?display=true&module=basket&page=basket_up_db"; } elseif($mode == "add") { echo $_SESSION['config']['businessappurl']."index.php?display=true&module=basket&page=basket_add_db"; } ?>" class="forms addforms">
				...
				<p>
					...
					<textarea  cols="30" rows="4"  name="basketclause" <?php if($_SESSION['m_admin']['basket']['basketId'] == 'CopyMailBasket' || $_SESSION['m_admin']['basket']['basketId'] == 'DepartmentBasket') { echo 'readonly="readonly" class="readonly"';} ?> id="basketclause" ><?php echo $_SESSION['m_admin']['basket']['clause']; ?></textarea> <a href="javascript::" onclick="window.open('<?php  echo $_SESSION['config']['businessappurl'];?>index.php?display=true&page=keywords_help&mode=popup','modify','toolbar=no,status=no,width=500,height=550,left=500,top=300,scrollbars=auto,location=no,menubar=no,resizable=yes');"><img src = "<?php  echo $_SESSION['config']['businessappurl'];?>static.php?filename=picto_menu_help.gif" alt="<? echo _HELP_KEYWORDS; ?>" title="<? echo _HELP_KEYWORDS; ?>" /></a>
				</p>
				<p class="buttons">
