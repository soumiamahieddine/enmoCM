...
<?php if($core_tools->test_service('display_basket_list','basket', false))
{
		...
		for ($i=0;$i<count($_SESSION['user']['baskets']);$i++)
		{
			...
			{
				echo '<li><a href="'.$_SESSION['config']['businessappurl'].'index.php?page=view_baskets&module=basket&baskets='.$_SESSION['user']['baskets'][$i]['id'].'"><img src="'.$_SESSION['config']['businessappurl'].'static.php?filename=nature_send.gif&module=basket" alt=""/> '.$_SESSION['user']['baskets'][$i]['name'].'  <b>'.$nb.'</b> </a></li>';
			}
		}
		?>
	</ul>
