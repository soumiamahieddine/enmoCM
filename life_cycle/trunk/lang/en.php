...
if (!defined('_ADMIN_LIFE_CYCLE')) {
    define(
        '_ADMIN_LIFE_CYCLE', 
        ' Administration des cycles de vie des '
...
if (!defined('_MANAGE_LC_CYCLE_STEPS')) {
    define(
        ...
        'life cycle steps management("lc_cycles_steps")'
    );
}
if (!defined('_MANAGE_LC_POLICIES')) {
    define(
        '_MANAGE_LC_POLICIES', 
        'Life cycle policies management ("lc_policies")'
    );
}
/*****************CYCLE_STEPS************************************/
...
if (!defined('_PI_COMMENT_ROOT')) {
    define(
        '_PI_COMMENT_ROOT', 
        'Packaging information: Archival Information Package (AIP) content '
        . 'and usage'
    );
}
...
if (!defined('_PI_COMMENT_CONTENT')) {
    define(
        '_PI_COMMENT_CONTENT', 
        'Digital resources in native format, each having a sequential' 
        . 'filename+extension'
    );
}
if (!defined('_PI_COMMENT_PDI')) {
    define(
        '_PI_COMMENT_PDI', 
        'Container for Preservation Description Information. This is a catalog '
        . 'containing descriptors for Provenance, Reference, Fixity, Context,'
        . 'and Access Rights for each resource in <CONTENT_FILE>. '
        . 'For convenience, history is stored apart in <HISTORY_FILE>. '
        .'See pdi.xsd for structure and comments'
    );
}
if (!defined('_HISTORY_COMMENT_ROOT')) {
    define(
        '_HISTORY_COMMENT_ROOT', 
        'Preservation Description Information - History : List of events on' 
        . 'resource, each event set being identified by resource filename in '
        . '<CONTENT_FILE>. Ordered by ascending date'
    );
}
if (!defined('_PDI_COMMENT_ROOT')) {
    define(
        '_PDI_COMMENT_ROOT', 'Preservation Description Information : List of '
        .'qualifiers for resources, ordered by Provenance, Reference, Fixity, '
        . 'Context, and Access Rights. There is one set by resource, each set '
        . 'being identified by resource filename in <CONTENT_FILE>'
    );
}
