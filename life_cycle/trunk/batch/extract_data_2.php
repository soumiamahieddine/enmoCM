...
/******************************************************************************/
    ...
    $querySelectedFile = "SELECT *  
                        FROM " . $GLOBALS['view'] . " WHERE (cast(res_id as character varying) IN (select distinct(record_id) as record_id from history where event_type like 'ACTION%' and cast(event_id as integer) in (select id FROM actions where id_status <> '_NOSTATUS') AND event_date > current_timestamp - interval '".$fromdatelist['FromDate']."')) AND destination = '".$fromdatelist['EntityId']."'";
    $stmt = Bt_doQuery(
        $GLOBALS['db'], 
        $querySelectedFile
    );
