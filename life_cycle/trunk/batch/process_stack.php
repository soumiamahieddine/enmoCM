	...
	switch($state) {
		...
		case "INIT" :
			...
			$argsparser->add_arg("adrTable", array (
				...
				'long' => "adr",
				'mandatory' => true,
				'help' => "Adress table target is mandatory."
			));
			...
			try {
				...
				if ($e->arg_name == "adrTable") {
					$GLOBALS['logger']->write('Adress table missing', 'ERROR', 1);
					$GLOBALS['exitCode'] = 1;
					$state = "END";break;
				}
		...
		case "GET_STEPS" :
			...
			if ($db->nb_result() == 0) {
				...
				while($stepsRecordset = $db->fetch_object()) {
					//array_push($GLOBALS['steps'], array("cycle_step_id" => $stepsRecordset->cycle_step_id, "step_operation" => $stepsRecordset->step_operation, "processed" => "KO"));
		...
		case "ACTION" :
			$db->connect();
			//do action
			$GLOBALS['logger']->write("current operation:".$GLOBALS['steps'][$currentStep]['step_operation'], 'INFO');
