...
/**
 * Get the batch if of the process stack
 * 
...
/**
 * Update the database with the new batch id of the process stack
 * 
 * @param object $db connection object to the database
 * @return nothing
 */
function updateWorkBatch() 
{
    $req = "update parameters set param_value_int  = " . $GLOBALS['wb'] . " "
         . "where id = 'lc_process_stack_id'";
    $GLOBALS['db']->query($req);
}

/**
 * exit the batch with a message in the log and an error log
 * 
 * @param int $errorCode error code to exit
 * @param string $errorMessage error message to the log
 * @return nothing, exit the program
 */
function exitBatch($errorCode, $errorMessage)
{
    ...
    fwrite($semaphore, "1");
    fclose($semaphore);
    $GLOBALS['logger']->write($errorMessage, 'ERROR', $errorCode);
    exit($errorCode);
}
...
try {
    ...
    MyInclude($GLOBALS['MaarchDirectory'] . 'core' . DIRECTORY_SEPARATOR 
              . 'core_tables.php');
    MyInclude($GLOBALS['MaarchDirectory'] . 'modules' .DIRECTORY_SEPARATOR 
...
$GLOBALS['errorLckFile'] = $GLOBALS['batchDirectory'] . DIRECTORY_SEPARATOR 
                         . $GLOBALS['policy'] . '_' . $GLOBALS['cycle'] 
                          . "_error.lck";
$GLOBALS['lckFile'] = $GLOBALS['batchDirectory'] . DIRECTORY_SEPARATOR 
                    . $GLOBALS['policy'] . '_' . $GLOBALS['cycle'] . ".lck";
if (file_exists($GLOBALS['errorLckFile'])) {
    $GLOBALS['logger']->write(
