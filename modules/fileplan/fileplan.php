	...
	<div id="inner_content">
		<div class="block">
			<h2><b>
				...
				&nbsp;/&nbsp;
				<a href="<?php echo $_SESSION['config']['businessappurl']; ?>index.php?page=fileplan_managment&module=fileplan&load" class="back">
			...
			<tr>
				<td style="width:25%;" valign="top" nowrap>
					...
					function TafelTreeInit () {
                        ...
                        for ($i = 0; $i < count($fileplans_array); ++$i) {
                            ...
                            $stmt = $db->query(
                                ...
                                .' and position_enabled = ? order by position_label asc', array($fileplans_array[$i]['ID'], 'Y'));
                            ...
                            while ($res = $stmt->fetchObject()) {
								...
								tree_<?php functions::xecho($fileplanId); ?> = new TafelTree('tree_fileplan_<?php functions::xecho($fileplanId); ?>', struct_<?php functions::xecho($fileplanId); ?>, {
									...
									'onOpenPopulate' : [funcOpen, '<?php echo $_SESSION['config']['businessappurl']; ?>index.php?display=true&module=fileplan&page=positions_tree_childs&fileplan_id=<?php 
					...
					function view_document_list( branch) {
						...
						// 	
						loadList('<?php echo $_SESSION['config']['businessappurl']; ?>index.php?display=true&module=fileplan&page=positions_documents_list&id='+id, 'list_doc', true);
					}
					</script>	
					<?php
                    for ($i = 0; $i < count($fileplans_array); ++$i) {
...
<?php
	...
	<div id="inner_content">
	<h1><i class="fa fa-copy" title="<?php
