...
switch ($mode) {
        ...
        if (strlen(trim($_REQUEST['fileplan_label'])) > 0) {
            ...
            if ($status <> 1) {
                ...
                $stmt = $db->query("INSERT INTO ".FILEPLAN_TABLE
                    ...
                    . " VALUES (?,?,?,?,?)", array($fileplan_label,$user_id,$entity_id,$_REQUEST['is_serial'],'Y'));
    ...
    case 'delFileplan':
        if (isset($_REQUEST['fileplan_id']) && !empty($_REQUEST['fileplan_id'])) {
            ...
            //Position tree array
            $content .='<b>'.functions::xssafe($fileplan_array['LABEL']).'</b><br/>';//Init with fileplan
            //Get positions tree
            ...
            for ($i=0; $i < count($positions_array); $i++) {
                ...
                if ($fileplan->isEnable($fileplan_array['ID'], $positions_array[$i]['ID'])) {
                    $content .='<b>'.str_replace("---", "&nbsp;&nbsp;", functions::xssafe(str_replace("&emsp;", "---", $positions_array[$i]['LABEL'])))
