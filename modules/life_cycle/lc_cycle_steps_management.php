    ...
    if ($state == false) {
        ...
        ?>
            ...
            action="<?php echo $_SESSION['config']['businessappurl']
                ...
                value="<?php if (isset($_REQUEST['order'])) {
                    functions::xecho($_REQUEST['order']);
                } ?>" />
                <input type="hidden" name="order_field" 
                id="order_field" 
                value="<?php if (isset($_REQUEST['order_field'])) {
        ...
        if ($mode == 'up') {
            echo '<p><a href="'.$_SESSION['config']['businessappurl']
                ...
                .'"><i class="fa fa-cogs fa-2x" title="'._VIEW_GENERAL_PARAMETERS_OF_THE_POLICY.' '
                ._LC_CYCLE_STEP.'"></i>&nbsp;'._VIEW_GENERAL_PARAMETERS_OF_THE_POLICY.'</a></p>'; ?>
                    <p>
                        ...
                        value="<?php if (isset($_SESSION['m_admin']['lc_cycle_steps']['policy_id'])) {
                    functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycle_steps']['policy_id']));
            ...
            for ($cptPolicies = 0; $cptPolicies < count($policiesArray); ++$cptPolicies) {
                ...
                if (isset($_SESSION['m_admin']['lc_cycle_steps']['policy_id'])
                    && $_SESSION['m_admin']['lc_cycle_steps']['policy_id'] == $policiesArray[$cptPolicies]
                ) {
        ...
        <?php 
        if (!empty($_SESSION['m_admin']['lc_cycle_steps']['policy_id']) && $mode == 'up'
        ) {
            ?>
                    <p>
                        <label for="cycle_id"><?php echo _CYCLE_ID; ?> : </label>
                        <input name="cycle_id" type="text"  id="cycle_id" value="<?php if (isset($_SESSION['m_admin']['lc_cycle_steps']['cycle_id'])) {
                functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycle_steps']['cycle_id']));
            } ?>" readonly='readonly' class='readonly'/>
                    </p>
        <?php
        } else {
            ...
            <?php
                ...
                <p>
                    ...
                    <input name="id" type="text"  id="id" value="<?php if (isset($_SESSION['m_admin']['lc_cycle_steps']['cycle_step_id'])) {
            functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycle_steps']['cycle_step_id']));
        } ?>" <?php if ($mode == 'up') {
            ...
            functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycle_steps']['cycle_step_desc']));
            ...
            echo $_SESSION['m_admin']['lc_cycle_steps']['cycle_step_desc'];
                ...
                <p>
                    ...
                    <select name="docserver_type_id" id="docserver_type_id">
                            ...
                            for ($cptDocserverTypes = 0; $cptDocserverTypes < count($docserverTypesArray); ++$cptDocserverTypes) {
                                ?>
                                <option value="<?php functions::xecho($docserverTypesArray[$cptDocserverTypes]); ?>" <?php if (isset($_SESSION['m_admin']['lc_cycle_steps']['docserver_type_id']) && $_SESSION['m_admin']['lc_cycle_steps']['docserver_type_id'] == $docserverTypesArray[$cptDocserverTypes]) {
                                    echo 'selected="selected"';
                                } ?>><?php functions::xecho($docserverTypesArray[$cptDocserverTypes]); ?></option>
                                <?php
                            } ?>
                    </select>
                </p>
                <!--<p>
                    ...
                    <input type="radio" class="check" name="is_allow_failure" value="true" <?php if (isset($_SESSION['m_admin']['docservers']['is_allow_failure']) && $_SESSION['m_admin']['docservers']['is_allow_failure']) {
                                ?> checked="checked"<?php
                            } ?> /><?php echo _YES; ?>
                    <input type="radio" class="check" name="is_allow_failure" value="false" <?php if (isset($_SESSION['m_admin']['docservers']['is_allow_failure']) && (!$_SESSION['m_admin']['docservers']['is_allow_failure'] || $_SESSION['m_admin']['docservers']['is_allow_failure'] == '')) {
                                ?> checked="checked"<?php
                            } elseif (!isset($_SESSION['m_admin']['docservers']['is_allow_failure'])) {
                                ?> checked="checked"<?php
                            } ?> /><?php echo _NO; ?>
                </p>-->
                <p>
                    ...
                    <select name="step_operation" id="step_operation">
                        ...
                        for ($cptStepOperation = 0; $cptStepOperation < count($_SESSION['lifeCycleFeatures']['LIFE_CYCLE']['PROCESS']['MODE']); ++$cptStepOperation) {
                            ?>
                            <option value="<?php if (isset($_SESSION['lifeCycleFeatures']['LIFE_CYCLE']['PROCESS']['MODE'][$cptStepOperation])) {
                                functions::xecho($_SESSION['lifeCycleFeatures']['LIFE_CYCLE']['PROCESS']['MODE'][$cptStepOperation]);
                            } ?>" <?php if (isset($_SESSION['m_admin']['lc_cycle_steps']['step_operation']) && $_SESSION['m_admin']['lc_cycle_steps']['step_operation'] == $_SESSION['lifeCycleFeatures']['LIFE_CYCLE']['PROCESS']['MODE'][$cptStepOperation]) {
                                echo 'selected="selected"';
                            } ?>><?php if (isset($_SESSION['lifeCycleFeatures']['LIFE_CYCLE']['PROCESS']['MODE'][$cptStepOperation])) {
                                echo $_SESSION['lifeCycleFeatures']['LIFE_CYCLE']['PROCESS']['MODE'][$cptStepOperation];
                            } ?></option>
                        <?php
                        } ?>
                    </select>
                ...
                <p>
                    ...
                    <input name="sequence_number" type="text"  id="sequence_number" value="<?php if (isset($_SESSION['m_admin']['lc_cycle_steps']['sequence_number'])) {
                            functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycle_steps']['sequence_number']));
                        } ?>" />
                </p>
                <p class="buttons">
                    <?php
