            ...
            <form name="formdocserver" method="post" class="forms" style="width:400px;" action="<?php echo $_SESSION['config']['businessappurl'].'index.php?display=true&page=lc_cycles_management_controler&module=life_cycle&mode='.$mode; ?>">
                ...
                if ($mode == 'up') {
                    ...
                    <p>
                        ...
                        <input name="policy_id" type="text"  id="policy_id" value="<?php if (isset($_SESSION['m_admin']['lc_cycles']['policy_id'])) {
                            functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycles']['policy_id']));
                    ...
                    <p>
                        ...
                        <select name="policy_id" id="policy_id">
                            ...
                            for ($cptPolicies = 0; $cptPolicies < count($policiesArray); ++$cptPolicies) {
                                ?>
                                <option value="<?php functions::xecho($policiesArray[$cptPolicies]); ?>" <?php if (isset($_SESSION['m_admin']['lc_cycles']['policy_id']) && $_SESSION['m_admin']['lc_cycles']['policy_id'] == $policiesArray[$cptPolicies]) {
                                    echo 'selected="selected"';
                                } ?>><?php if (isset($policiesArray[$cptPolicies])) {
                                    functions::xecho($policiesArray[$cptPolicies]);
                                } ?></option>
                                <?php
                            } ?>
                        </select>
                ...
                <p>
                    ...
                    <input name="id" type="text"  id="id" value="<?php if (isset($_SESSION['m_admin']['lc_cycles']['cycle_id'])) {
                    functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycles']['cycle_id']));
                } ?>" <?php if ($mode == 'up') {
                ...
                <p>
                    ...
                    <textarea name="cycle_desc" type="text"  id="cycle_desc" value="<?php if (isset($_SESSION['m_admin']['lc_cycles']['cycle_desc'])) {
                    functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycles']['cycle_desc']));
                } ?>" /><?php if (isset($_SESSION['m_admin']['lc_cycles']['cycle_desc'])) {
                    echo $_SESSION['m_admin']['lc_cycles']['cycle_desc'];
                } ?></textarea>
                </p>
                <p>
                    <label for="sequence_number"><?php echo _SEQUENCE_NUMBER; ?> : </label>
                    <input name="sequence_number" type="text"  id="sequence_number" value="<?php if (isset($_SESSION['m_admin']['lc_cycles']['sequence_number'])) {
                    functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycles']['sequence_number']));
                } ?>" />
                </p>
                <p>
                    <label for="where_clause"><?php echo _WHERE_CLAUSE; ?> : </label>
                    <textarea name="where_clause" type="text"  id="where_clause" value="<?php if (isset($_SESSION['m_admin']['lc_cycles']['where_clause'])) {
                    functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycles']['where_clause']));
                } ?>" /><?php if (isset($_SESSION['m_admin']['lc_cycles']['where_clause'])) {
                    echo $_SESSION['m_admin']['lc_cycles']['where_clause'];
                } ?></textarea>
                </p>
                <p>
                    <label for="break_key"><?php echo _BREAK_KEY; ?> : </label>
                    <input name="break_key" type="text"  id="break_key" value="<?php if (isset($_SESSION['m_admin']['lc_cycles']['break_key'])) {
                    functions::xecho($func->show_str($_SESSION['m_admin']['lc_cycles']['break_key']));
                } ?>" />
                </p>
