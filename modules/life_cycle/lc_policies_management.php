...
if ($mode == 'list') {
        ...
        if ($state == false) {
            ...
            <form id="adminform" method="post" class="forms" style="width:400px;" action="<?php echo $_SESSION['config']['businessappurl'].'index.php?display=true&page=lc_policies_management_controler&module=life_cycle&mode='.$mode; ?>">
                ...
                <p>
                    ...
                    <input name="id" type="text"  id="id" value="<?php if (isset($_SESSION['m_admin']['lc_policies']['policy_id'])) {
                functions::xecho($func->show_str($_SESSION['m_admin']['lc_policies']['policy_id']));
            } ?>" <?php if ($mode == 'up') {
                ...
                <p>
                    ...
                    <input name="policy_name" type="text"  id="policy_name" value="<?php if (isset($_SESSION['m_admin']['lc_policies']['policy_name'])) {
                functions::xecho($func->show_str($_SESSION['m_admin']['lc_policies']['policy_name']));
            } ?>" />
                </p>
                <p>
                    <label for="policy_desc"><?php echo _POLICY_DESC; ?> : </label>
                    <textarea name="policy_desc" type="text"  id="policy_desc" value="<?php if (isset($_SESSION['m_admin']['lc_policies']['policy_desc'])) {
                functions::xecho($func->show_str($_SESSION['m_admin']['lc_policies']['policy_desc']));
    ...
    if (
        ...
        if ($_SESSION['m_admin']['lc_policies']['tabWf']['status'] != 'ko') {
            ...
            for (
                    $cptCycles = 0;
                    $cptCycles < count($_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles']);
                    ++$cptCycles
                ) {
                ...
                $content .= '<a href="'.$_SESSION['config']['businessappurl']
                            ...
                            .'module=life_cycle&id='.$_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['cycle_id'].'">';
                $content .= '<i class="fa fa-cogs fa-2x" title="'._SETUP.' '
                            ._LC_CYCLE.'"></i></a>&nbsp;';
                $content .= $link;
                $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['cycle_id'];
                $content .= '</a>';
                $content .= '</td>';
                $content .= '<td align="left" width="300px">';
                $content .= $link;
                $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['cycle_desc'];
                $content .= '</a>';
                $content .= '</td>';
                $content .= '<td align="left" width="300px">';
                $content .= $link;
                $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['where_clause'];
                $content .= '</a>';
                $content .= '</td>';
                $content .= '<td align="left" width="100px">';
                $content .= $link;
                $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['break_key'];
                $content .= '</a>';
                $content .= '</td>';
                $content .= '<td align="right" width="100px">';
                $content .= $link;
                $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['sequence_number'];
                $content .= '</a>';
                $content .= '&nbsp;</td>';
                $content .= '</tr>';
                $content .= '</table>';
                ...
                for (
                        $cptSteps = 0;
                        $cptSteps < count($_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps']);
                        ++$cptSteps
                    ) {
                    ...
                    $content .= '<a href="'.$_SESSION['config']['businessappurl']
                                    ...
                                    .'module=life_cycle&id='.$_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps'][$cptSteps]['cycle_step_id'].'">';
                    $content .= '<i class="fa fa-cogs fa-2x" title="'._SETUP.' '
                                         ._LC_CYCLE_STEP.'"></i></a>&nbsp;';
                    $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps'][$cptSteps]['cycle_step_id'];
                    $content .= '</small></td>';
                    $content .= '<td align="left" width="300px"><small>';
                    $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps'][$cptSteps]['cycle_step_desc'];
                    $content .= '</small></td>';
                    $content .= '<td align="left" width="200px"><small>';
                    $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps'][$cptSteps]['docserver_type_id'];
                    $content .= '</small></td>';
                    $content .= '<td align="left" width="150px"><small>';
                    $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps'][$cptSteps]['step_operation'];
                    $content .= '</small></td>';
                    $content .= '<td align="right" width="80px"><small>';
                    $content .= $_SESSION['m_admin']['lc_policies']['tabWf']['value']['cycles'][$cptCycles]['steps'][$cptSteps]['sequence_number'];
                    $content .= '&nbsp;</small></td>';
                    $content .= '</tr>';
                    $content .= '</table>';
                }
