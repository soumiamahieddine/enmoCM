...
export class PrivilegeService {
  ...
  getAllPrivileges(getLockedPrivilege: boolean, authMode: string = "standard") {
    ...
    priv = priv.filter(
      (elem) =>
        this.getExcludePrivilege(getLockedPrivilege, authMode).indexOf(elem) ===
        -1
    );
    ...
    if (ids !== null) {
      menus = this.menus.filter(
        (elem) =>
          ...
          this.headerService.user.privileges.indexOf(elem.id) > -1
      );
    } else {
      menus = this.menus.filter(
        (elem) => this.headerService.user.privileges.indexOf(elem.id) > -1
      );
    }

    if (
      this.headerService.user.groups.filter(
        (group: any) => group.can_index === true
      ).length > 0 &&
      (ids === null || ids.indexOf("indexing") > -1)
    ) {
      const indexingGroups: any[] = [];

      this.headerService.user.groups
  ...
  resfreshUserShortcuts() {
    ...
    this.shortcuts = this.shortcuts.concat(
      this.menus
        .filter((elem) => elem.shortcut === true)
        .filter(
          (elem) => this.headerService.user.privileges.indexOf(elem.id) > -1
        )
    );
  ...
  getCurrentUserAdministrationsByUnit(unit: string): Array<administration> {
    if (
      this.hasCurrentUserPrivilege("view_history") &&
      this.hasCurrentUserPrivilege("view_history_batch")
    ) {
      return this.administrations
        .filter((elem) => elem.unit === unit)
        .filter(
          (elem) => this.headerService.user.privileges.indexOf(elem.id) > -1
        )
        .filter((priv) => priv.id !== "view_history_batch");
    } else {
      return this.administrations
        .filter((elem) => elem.unit === unit)
        .filter(
