...
export class ListAdministrationComponent implements OnInit {
  ...
  availableData: any = [
    ...
    },
    {
      ...
      sample: this.translate.instant(
        "lang.getCreationAndProcessLimitDatesSample"
      ),
      cssClasses: ["align_leftData"],
      icon: "fa-calendar",
    },
    ...
    },
    {
      ...
      sample:
        '<i color="accent" class="fa fa-check"></i> Barbara BAIN -> <i class="fa fa-hourglass-half"></i> <b>Bruno BOULE</b> -> <i class="fa fa-hourglass-half"></i> Patricia PETIT',
      cssClasses: ["align_leftData"],
      icon: "fa-list-ol",
    },
    {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.filteredDataOptions = this.dataControl.valueChanges.pipe(
      startWith(""),
    ...
    this.basketGroup.list_display.subInfos.forEach((element: any) => {
      if (element !== undefined) {
        ...
        this.displayedSecondaryData[
          this.displayedSecondaryData.length - 1
    ...
    if (this.basketGroup.list_event === "processDocument") {
      this.selectedProcessTool.defaultTab =
        this.basketGroup.list_event_data === null
          ? "dashboard"
          : this.basketGroup.list_event_data.defaultTab;
      this.selectedProcessTool.canUpdateData =
        this.basketGroup.list_event_data === null
          ? false
          : this.basketGroup.list_event_data.canUpdateData;
      this.selectedProcessTool.canUpdateModel =
        this.basketGroup.list_event_data === null
          ? false
          : this.basketGroup.list_event_data.canUpdateModel;
    } else if (this.basketGroup.list_event === "signatureBookAction") {
      this.selectedProcessTool.canUpdateDocuments =
        this.basketGroup.list_event_data === null
          ? false
          : this.basketGroup.list_event_data.canUpdateDocuments;
    }

    this.selectedProcessToolClone = JSON.parse(
      JSON.stringify(this.selectedProcessTool)
    );
    this.displayedSecondaryDataClone = JSON.parse(
      JSON.stringify(this.displayedSecondaryData)
    );
  }
  ...
  addData(id: any) {
    ...
    this.displayedSecondaryData.push(
      this.availableData.filter((item: any) => item.value === id)[0]
    );
    ...
    if (event.previousContainer === event.container) {
      ...
      this.displayedSecondaryData.forEach((subArray: any, index) => {
        if (subArray.length > this.selectedTemplateDisplayedSecondaryData) {
          transferArrayItem(
            subArray,
            this.displayedSecondaryData[index + 1],
            subArray.length,
            0
          );
        } else if (
          subArray.length < this.selectedTemplateDisplayedSecondaryData &&
          !this.functions.empty(this.displayedSecondaryData[index + 1])
        ) {
          transferArrayItem(
            this.displayedSecondaryData[index + 1],
            subArray,
  ...
  saveTemplate() {
    ...
    this.http
      .put(
        "../rest/baskets/" +
          this.basketGroup.basket_id +
          "/groups/" +
          this.basketGroup.group_id,
        {
          list_display: objToSend,
          list_event: this.selectedListEvent,
          list_event_data: this.selectedProcessTool,
        }
      )
      .pipe(
        tap(() => {
          ...
          this.selectedTemplateDisplayedSecondaryDataClone = JSON.parse(
            JSON.stringify(this.selectedTemplateDisplayedSecondaryData)
          );

          this.notify.success(
            this.translate.instant("lang.modificationsProcessed")
          );
  ...
  private _filterData(value: any): string[] {
    ...
    return this.availableData.filter((option: any) =>
  ...
  checkModif() {
    if (
      JSON.stringify(this.displayedSecondaryData) ===
        JSON.stringify(this.displayedSecondaryDataClone) &&
      this.selectedListEvent === this.selectedListEventClone &&
      JSON.stringify(this.selectedProcessTool) ===
        JSON.stringify(this.selectedProcessToolClone) &&
      JSON.stringify(this.selectedTemplateDisplayedSecondaryData) ===
  ...
  cancelModification() {
    this.displayedSecondaryData = JSON.parse(
      JSON.stringify(this.displayedSecondaryDataClone)
    );
    this.selectedListEvent = this.selectedListEventClone;
    this.selectedProcessTool = JSON.parse(
      JSON.stringify(this.selectedProcessToolClone)
    );
    this.availableData = JSON.parse(JSON.stringify(this.availableDataClone));
    this.selectedTemplateDisplayedSecondaryData = JSON.parse(
      JSON.stringify(this.selectedTemplateDisplayedSecondaryDataClone)
    );
    this.dataControl.setValue("");
  }
