...
class entity extends dbquery
{
	...
	*/
	public function formentity($mode, $id = '')
	{
		...
		if($mode == "up")
		{
			...
			{
				$this->connect();
				$this->query('select * from '.$_SESSION['tablename']['ent_entities']." where entity_id = '".$id."'");
				if($this->nb_result() == 0)
				{
	...
	*/
	public function havechild($id)
	{
		...
		$this->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where parent_entity_id = '".$id."'");
	...
	*/
	public function isRelated($id)
	{
		...
		$this->query('select ue.entity_id from '.$_SESSION['tablename']['ent_users_entities']." ue, ".$_SESSION['tablename']['users']." u where ue.user_id = u.user_id and ue.entity_id = '".$id."'");
	...
	*/
	public function getentitylabel($entity_id)
	{
		...
		$this->query("select entity_label from ".$_SESSION['tablename']['ent_entities']." where entity_id = '".$entity_id."'");
		//$this->show();
		if($this->nb_result() > 0)
		{
			$line = $this->fetch_object();
	...
	*/
	...
	{
		$this->connect();
		$this->query('select entity_id, entity_label from '.$_SESSION['tablename']['ent_entities']." where enabled = 'Y' and entity_id = '".$entity_id."'");
		if($this->nb_result() > 0)
		{
			$line = $this->fetch_object();
			return $line;
	...
	*/
	...
	{
		$this->connect();
		$this->query('select entity_id, entity_label from '.$_SESSION['tablename']['ent_entities']." where enabled = 'Y' and parent_entity_id = '".$parent."'".$where);
		...
		if($this->nb_result() > 0)
		{
			...
			while($line = $this->fetch_object())
			{
				...
				{
					 array_push($entities, array('ID' =>$line->entity_id, 'LABEL' =>  $espace.$this->show_string($line->entity_label), 'KEYWORD' => false));

					$db2 = new entity();
					$db2->connect();
					$db2->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where enabled = 'Y' and parent_entity_id = '".$line->entity_id."'".$where);
					$tmp = array();
					if($db2->nb_result() > 0)
					{
	...
	*/
	public function getTabChildrenId($parent = '', $where = '', $immediate_children_only = false)
	{
		...
		$this->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where parent_entity_id = '".$parent."'".$where);

		if($this->nb_result() > 0)
		{
			while($line = $this->fetch_object())
			{
				$tab_children_id[] = "'".$line->entity_id."'";

				if($immediate_children_only == false)
				{
					$db2 = new entity();
					$db2->connect();
					$db2->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where parent_entity_id = '".$line->entity_id."'".$where);
					if($db2->nb_result() > 0)
					{
	...
	*/
	public function get_entities_of_user($user_id,$parent = 'all')
	{
		...
		$db = new dbquery();
		$db->connect();
		$db->query("select e.entity_id,e.entity_label, ue.primary_entity, ue.user_role from ".$_SESSION['tablename']['ent_entities']." e, ".$_SESSION['tablename']['ent_users_entities']." ue where ue.entity_id = e.entity_id and ue.user_id = '".$db->protect_string_db($user_id)."' order by e.entity_label");
		while($res = $db->fetch_object())
		{
			array_push($entities, array('ID' => $res->entity_id, 'LABEL' => $res->entity_label, 'PRIMARY' => $res->entity_label, 'ROLE' => $res->user_role ));
		}
		return $entities;
	}
	/**
	* Checks if an user can update an entity
	*
	* @param string $user_id user identifier
	* @param string $entity_id entity identifier
	*/
	/*public function access_user_entity($user_id, $entity_id)
	{
	...
	*/
	public function allowbanentity($id, $mode)
	{
		...
		$this->connect();
		$this->query('Update '.$_SESSION['tablename']['ent_entities']." set enabled = '".$action."' where entity_id = '".$id."'");
		...
		if($this->nb_result() > 0)
		{
			while($line = $this->fetch_object())
			{
				...
				$db2->query('Update '.$_SESSION['tablename']['ent_entities']." set enabled = '".$action."' where entity_id = '".$line->entity_id."'");
				if($_SESSION['history'][hist] == "true")
				{
					require_once($_SESSION['pathtocoreclass']."class_history.php");
					$users = new history();
				...
				$db2->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where parent_entity_id = '".$line->entity_id."'");
				if($db2->nb_result() > 0)
				{
	...
	*/
	public function adminentity($id, $mode)
	{
		...
		else
		{
			...
			$this->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where entity_id = '".$id."'");
			...
			else
			{
				...
				elseif($mode == 'del' )
				{
					...
					elseif($this->isRelated($id))
					{
						...
						$this->query("delete from ".$_SESSION['tablename']['ent_entities']." where entity_id = '".$id."'");
						if($_SESSION['history']['entitydel'] == "true")
						{
							require_once($_SESSION['pathtocoreclass'].'class_history.php');
							$users = new history();
							$users->add($_SESSION['tablename']['ent_entities'], $id,'DEL',_ENTITY_DELETION." : ".$id, $_SESSION['config']['databasetype']);
						}
	...
	*/
	public function addupentity($mode)
	{
		...
		else
		{
			...
			if($mode == 'add')
			{
				$this->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where entity_id = '".$_SESSION['m_admin']['entity']['entityId'] ."'");
				if($this->nb_result() > 0)
				{
				...
				{
					$this->query('INSERT INTO '.$_SESSION['tablename']['ent_entities']." (entity_id, entity_label, adrs_1, adrs_2, adrs_3, zipcode, city, country, email, business_id, parent_entity_id, entity_type) VALUES ('".$_SESSION['m_admin']['entity']['entityId']."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['label'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['adrs1'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['adrs2'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['adrs3'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['zcode'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['city'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['country'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['email'])."', '".$this->protect_string_db($_SESSION['m_admin']['entity']['business'])."', '".$_SESSION['m_admin']['entity']['parent']."', '".$_SESSION['m_admin']['entity']['type']."')");
			...
			elseif($mode == 'up')
			{
				$this->query('UPDATE '.$_SESSION['tablename']['ent_entities']." set entity_label = '".$this->protect_string_db($_SESSION['m_admin']['entity']['label'])."' , adrs_1 = '".$this->protect_string_db($_SESSION['m_admin']['entity']['adrs1'])."', adrs_2 = '".$this->protect_string_db($_SESSION['m_admin']['entity']['adrs2'])."', adrs_3 = '".$this->protect_string_db($_SESSION['m_admin']['entity']['adrs3'])."', zipcode = '".$this->protect_string_db($_SESSION['m_admin']['entity']['zcode'])."', city = '".$this->protect_string_db($_SESSION['m_admin']['entity']['city'])."', country = '".$this->protect_string_db($_SESSION['m_admin']['entity']['country'])."', email = '".$this->protect_string_db($_SESSION['m_admin']['entity']['email'])."', business_id = '".$this->protect_string_db($_SESSION['m_admin']['entity']['business'])."', parent_entity_id = '".$_SESSION['m_admin']['entity']['parent']."', entity_type = '".$_SESSION['m_admin']['entity']['type']."' where entity_id = '".$_SESSION['m_admin']['entity']['entityId'] ."'");
	...
	*/
	public function get_entity_type_level($entity_id)
	{
		...
		$this->connect();
		$this->query('select entity_id, entity_label, entity_type from '.$_SESSION['tablename']['ent_entities']." where entity_id  = '".$entity_id."'");
		//$this->show();
		$line = $this->fetch_object();
	...
	*/
	public function getParentEntityId($entity_id)
	{
		...
		{
			$this->connect();
			$this->query("select parent_entity_id from ".$_SESSION['tablename']['ent_entities']." where entity_id = '".$entity_id."'");
	...
	public function getTabSisterEntityId($entity_id)
	{
		...
		if(!empty($entity_id))
		{
			...
			if(!empty($parent))
			{
				$this->query('select entity_id from '.$_SESSION['tablename']['ent_entities']." where parent_entity_id = '".$parent."' and entity_id <> '".$entity_id."'");
	...
	public function is_user_in_entity($user_id, $entity_id)
	{
		...
		{
			$this->connect();
			$this->query("select entity_id from ".$_SESSION['tablename']['ent_users_entities']." where user_id = '".$this->protect_string_db($user_id)." and entity_id = '".$this->protect_string_db($entity_id)."'");
			if($this->nb_result() == 1)
			{
				return true	;
			}
	...
	public function get_primary_entity($user_id)
	{
		...
		$this->connect();
		$this->query("select ue.entity_id, ue.user_role, e.entity_label from ".$_SESSION['tablename']['ent_entities']." e, ".$_SESSION['tablename']['ent_users_entities']." ue where ue.user_id = '".$this->protect_string_db($user_id)."' and ue.entity_id = e.entity_id and ue.primary_entity = 'Y'");
		$res = $this->fetch_object();
		return array('ID' => $res->entity_id, 'LABEL' => $res->entity_label, 'ROLE' => $res->user_role);
	}
}
?>
