...
if(isset($_GET['id']))
{
	$id = addslashes($db->wash($_GET['id'], "no", _THE_CONTACT_PURPOSE));
}
else
{
	$id = "";
...
if ($_REQUEST['valid']) {
	...
	if ($_POST['new']){
		...
		$db->query("DELETE FROM ".$_SESSION['tablename']['contact_purposes']." WHERE id = ".$id);
		$db->query("UPDATE ".$_SESSION['tablename']['contact_addresses']." SET contact_purpose_id = ".$newid." WHERE contact_purpose_id = ".$id);
		...
		?>
	        <script type="text/javascript">
	            window.location.href="<?php echo $_SESSION['config']['businessappurl'].'index.php?page=contact_purposes&order='.$_REQUEST['order'].'&order_field='.$_REQUEST['order_field'].'&start='.$_REQUEST['start'].'&what='.$_REQUEST['what'];?>";
	        </script>	
	    <?php
	} else {
