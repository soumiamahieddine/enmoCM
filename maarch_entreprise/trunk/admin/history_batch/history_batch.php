...
$tab = array();
$modules = array();
$db->query("select DISTINCT module_name from ".$_SESSION['tablename']['history_batch']);
while($res = $db->fetch_object())
{
	...
	elseif(isset($_SESSION['modules_loaded'][$res->module_name]['comment']) && !empty($_SESSION['modules_loaded'][$res->module_name]['comment']))
	{
		array_push($modules, array('id' => $res->module_name, 'label' => $_SESSION['modules_loaded'][$res->module_name]['comment']));
	}
	else
	{
		array_push($modules, array('id' => $res->module_name, 'label' => $res->module_name));
	}
}
if($_REQUEST['search'] ||
	 (!empty($_SESSION['m_admin']['history_batch_action'])&& isset($_SESSION['m_admin']['history_batch_action']))
  || (!empty($_SESSION['m_admin']['history_batch_module'])&& isset($_SESSION['m_admin']['history_batch_module']))
  || (!empty($_SESSION['m_admin']['onlyerrors'])&& isset($_SESSION['m_admin']['onlyerrors']))
  ...
  )
{
	...
	if((isset($_REQUEST['module']) && !empty($_REQUEST['module'])) || (!empty($_SESSION['m_admin']['history_batch_module'])&& isset($_SESSION['m_admin']['history_batch_module'])))
	{
		if(isset($_REQUEST['module']) && !empty($_REQUEST['module']))
		{
			$_SESSION['m_admin']['history_batch_module'] = $_REQUEST['module'];
		}
		$where .= "  ".$_SESSION['tablename']['history_batch'].".module_name = '".$_SESSION['m_admin']['history_batch_module']."' and";
		$_SESSION['m_admin']['history_batch_module'] = "";
	}
	if((isset($_REQUEST['onlyerrors']) && !empty($_REQUEST['onlyerrors'])) || (!empty($_SESSION['m_admin']['onlyerrors'])&& isset($_SESSION['m_admin']['onlyerrors'])))
	{
		...
		{
			$where .= "  ".$_SESSION['tablename']['history_batch'].".total_errors > 0 and";
		}
		$_SESSION['m_admin']['onlyerrors'] = "";
	}
	if((isset($_REQUEST['datestart']) && !empty($_REQUEST['datestart']))|| (!empty($_SESSION['m_admin']['history_batch_datestart'])&& isset($_SESSION['m_admin']['history_batch_datestart'])) )
	{
		if( preg_match($pattern,$_REQUEST['datestart'])==false && (!isset($_SESSION['m_admin']['history_batch_datestart']) || empty($_SESSION['m_admin']['history_batch_datestart'])))
		{
		...
		else
		{
			...
			$where .= " (".$_SESSION['tablename']['history_batch'].".event_date >= '".$_SESSION['m_admin']['history_batch_datestart']."') and ";
			$_SESSION['m_admin']['history_batch_datestart'] = "";
		}
	}
	if((isset($_REQUEST['dateend']) && !empty($_REQUEST['dateend']))|| (!empty($_SESSION['m_admin']['history_batch_dateend'])&& isset($_SESSION['m_admin']['history_batch_dateend'])) )
	{
		if( preg_match($pattern,$_REQUEST['dateend'])==false && (!isset($_SESSION['m_admin']['history_batch_dateend']) || empty($_SESSION['m_admin']['history_batch_dateend'])) )
		{
			$_SESSION['error'] = _DATE.' '._WRONG_FORMAT;
		}
		else
		{
			...
			$where .= " ( ".$_SESSION['tablename']['history_batch'].".event_date <= '".$_SESSION['m_admin']['history_batch_dateend']."') and ";
...
array_push($select[$_SESSION['tablename']['history_batch']],"id", "event_date", "batch_id", "module_name", "total_processed", "total_errors", "info" );
...
$req = new request();
$tab = $req->select($select, $where, $orderstr, $_SESSION['config']['databasetype'], $limit="500",false,$_SESSION['tablename']['history_batch']);
//$req->show();
...
<div id="search_hist" class="block">
	<form name="search_hist" action="<?php  echo $_SESSION['config']['businessappurl'];?>index.php?page=history_batch&amp;admin=history_batch" method="post" class="forms">
		...
		<p>
			...
			<select name="module">
				...
				{
					?>
					<option value="<?php  echo $modules[$i]['id'];?>"><?php  echo $modules[$i]['label'];?></option>
					<?php
				}
				?>
			</select>
