...
if($mode == "list")
{
    list_show::admin_list($tab, $i, $title, 'id','status_management_controler&mode=list','status','id', true, $page_name_up, $page_name_val, $page_name_ban, $page_name_del, $page_name_add, $label_add, FALSE, FALSE, _ALL_STATUS, _STATUS_SING, $_SESSION['config']['businessappurl'].'static.php?filename=manage_status_b.gif', false, true, false, true, $what, true, $autoCompletionArray);
}
elseif($mode == "up" || $mode == "add")
{
    ...
    <?php
    if($state == false)
        echo "<br /><br /><br /><br />"._THE_STATUS.' '._UNKNOWN."<br /><br /><br /><br />";
    else
    {?>
        <form name="frmstatus" id="frmstatus" method="post" action="<?php echo $_SESSION['config']['businessappurl']."index.php?display=true&admin=status&page=status_management_controler&mode=".$mode;?>" class="forms addforms">
            ...
            <p>
                ...
                <input name="status_id" type="text"  id="status_id" value="<?php echo functions::show_str($_SESSION['m_admin']['status']['ID']); ?>" <?php if($mode == "up"){ echo 'readonly="readonly" class="readonly"';}?>/>
            </p>
            <p>
                <label for="label"><?php echo _DESC; ?> : </label>
                <input name="label" type="text"  id="label" value="<?php echo functions::show_str($_SESSION['m_admin']['status']['LABEL']); ?>"/>
            </p>
            <p>
            ...
            if($mode == "up")
                ...
                ?>
               <input type="button" class="button"  name="cancel" value="<?php echo _CANCEL; ?>" onclick="javascript:window.location.href='<?php echo $_SESSION['config']['businessappurl'];?>index.php?page=status_management_controler&amp;mode=list&amp;admin=status';"/>
            </p>
        </form >
