	...
	{?>
		<form name="frmstatus" id="frmstatus" method="post" action="<?php echo $_SESSION['config']['businessappurl']."index.php?display=true&admin=status&page=status_management_controler&mode=".$mode;?>" class="forms addforms">
			...
			<p>
				...
				<input name="status_id" type="text"  id="status_id" value="<?php echo functions::show($_SESSION['m_admin']['status']['ID']); ?>" <?php if($mode == "up"){ echo 'readonly="readonly" class="readonly"';}?>/>
			</p>
			<p>
				<label for="label"><?php echo _DESC; ?> : </label>
				<input name="label" type="text"  id="label" value="<?php echo functions::show($_SESSION['m_admin']['status']['LABEL']); ?>"/>
			</p>
			<p>
