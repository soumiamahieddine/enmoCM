...
{
    $_SESSION['auth_dep']['bask_chosen_viewed'] = "none";
    $_REQUEST['isViewed'] = "none";
}
...
if(($_REQUEST['isViewed'] <> "none" && $_REQUEST['isViewed'] <> ""))
{
    $_SESSION['auth_dep']['bask_chosen_viewed'] = trim($_REQUEST['isViewed']);
}
...
if(isset($_SESSION['auth_dep']['bask_chosen_viewed']) && !empty($_SESSION['auth_dep']['bask_chosen_viewed']))
{
    if($where_concat <> "")
    {
...
if(($_REQUEST['template']== 'group_case_for_basket') && ($core_tools->is_module_loaded('cases')))
{
    ...
    $select[$table] = array();
    array_push($select[$table], "priority", "creation_date", "res_id");
...
else
{
    if($_SESSION['current_basket']['basket_owner'] <> "") {
        ...
        $where_concat = $table.".res_id = ".$_SESSION['tablename']['ent_listinstance'].".res_id and (" . $where_concat . ") and (".$_SESSION['tablename']['ent_listinstance'].".item_id = '".$_SESSION['user']['UserId']."' ".(!empty($entities)?"or ".$_SESSION['tablename']['ent_listinstance'].".item_id in (" . $entities . ")":"").")";
    }
    ...
    $nb_persitent=0;
    while ($return_dbQuery = $db->fetch_object()) {
        $tab2[$nb_persitent] = $return_dbQuery->res_id;
        $nb_persitent++;
    }
}
//$request->show();
...
$template_list=array();
array_push($template_list, array("name"=>"copy_list_extend", "img"=>"extend_list.gif", "label"=> _ACCESS_LIST_EXTEND));
            ...
            foreach(array_keys($tab[$i][$j]) as $value)
            {
                if($tab[$i][$j][$value]=="res_id")
                {
                    ...
                    for ($z=0; $z<count($tab2); $z++) {
                        if ($tab[$i][$j]['value'] == $tab2[$z]) {
                            $tab[$i][$j]['isPersistent'] = true;
                        }
                    }
                }
