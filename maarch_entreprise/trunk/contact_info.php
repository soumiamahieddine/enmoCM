...
if (!empty($_REQUEST['submit'])) {
    ...
    if (! empty($_SESSION['error'])) {
        ...
        if($_REQUEST['mode'] == 'add') {
            ...
            // Update
            $db->query(
                "UPDATE " . $_SESSION['tablename']['contacts'] 
                . " SET "
    ...
    // Close after submit without error
...
// Load from database (view/update)
else if($_REQUEST['id'] && ($_REQUEST['mode']== 'view' || $_REQUEST['mode']== 'up'))
{
    ...
    else
    {
        ...
        $contact['ID'] = $line->contact_id;
        $contact['TITLE'] = $line->title;
        $contact['TITLE_LABEL'] = $business->get_label_title($line->title);
        $contact['LASTNAME'] = $func->show_string($line->lastname);
        $contact['FIRSTNAME'] = $func->show_string($line->firstname);
        $contact['SOCIETY'] = $func->show_string($line->society);
        $contact['FUNCTION'] = $func->show_string($line->function);
        $contact['ADD_NUM'] = $func->show_string($line->address_num);
        $contact['ADD_STREET'] = $func->show_string($line->address_street);
        $contact['ADD_COMP'] = $func->show_string($line->address_complement);
        $contact['ADD_TOWN'] = $func->show_string($line->address_town);
        $contact['ADD_CP'] = $func->show_string($line->address_postal_code);
        $contact['ADD_COUNTRY'] = $func->show_string($line->address_country);
        $contact['PHONE'] = $func->show_string($line->phone);
...
<div id="contacts" style="padding-left: 20px;">
    <h2 align="center"><img src="<?php echo $_SESSION['config']['businessappurl'];?>static.php?filename=my_contacts_off.gif" alt="<?php echo _CONTACT_INFO;?>" /> <?php echo _CONTACT_INFO;?> N°<?php echo $contact['ID']; ?></h2>  <br/>
    <form name="frmcontact" id="frmcontact" method="post" action="<?php echo $_SESSION['config']['businessappurl'];?>index.php?display=true&page=contact_info" class="forms addforms">
        ...
        <p>
            ...
            <?php if($readonly){
                if($contact['IS_CORPORATE_PERSON'] == 'Y'){echo _YES;}else{echo _NO;}
            } else
            { ?>
            <input type="radio"  class="check" name="is_corporate" value="Y" <?php if($contact['IS_CORPORATE_PERSON'] == 'Y'){?> checked="checked"<?php }elseif(empty($contact['IS_CORPORATE_PERSON'])){ ?> checked="checked"<?php } ?> onclick="javascript:show_admin_contacts(true);"/><?php echo _YES;?>
            <input type="radio"  class="check" name="is_corporate" value="N" <?php if($contact['IS_CORPORATE_PERSON'] == 'N'){?> checked="checked"<?php } ?> onclick="javascript:show_admin_contacts( false);"/><?php echo _NO;
            } ?>
        </p>
         <p id="title_p" style="display:<?php if($contact['IS_CORPORATE_PERSON'] == 'Y'){ echo 'none';} else { echo 'block'; }?>;">
            <label for="title"><?php echo _TITLE2; ?> : </label>
            <?php if($readonly)
            {
                ?><input name="title" type="text"  id="title" value="<?php echo $func->show_str($contact['TITLE_LABEL']); ?>" readonly="readonly"/><?php
            }
            else
            {?>
            <select name="title" id="title">
                ...
                <?php foreach(array_keys($titles) as $key)
                {
                    ?><option value="<?php echo $key;?>" <?php 
                        if(($_REQUEST['mode'] == 'add' && $key == $defaultTitle) || $key == $contact['TITLE'] ){ echo 'selected="selected"';}?>><?php echo $titles[$key];?></option><?php
                }?>
            </select>
            <?php }?>

         </p>
         <p id="lastname_p" style="display:<?php if($contact['IS_CORPORATE_PERSON'] == 'Y'){ echo 'none';}else{ echo 'block';}?>;">
            <label for="label"><?php echo _LASTNAME; ?> : </label>
            <input name="lastname" type="text"  id="lastname" value="<?php echo $func->show_str($contact['LASTNAME']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?> /> <?php if(!$readonly){?><span class="red_asterisk" id="lastname_mandatory" style="display:none;">*</span><?php } ?>
         </p>
         <p id="firstname_p" style="display:<?php if($contact['IS_CORPORATE_PERSON'] == 'Y'){ echo 'none';}else{ echo 'block';}?>;">
            <label for="label"><?php echo _FIRSTNAME; ?> : </label>
            <input name="firstname" type="text"  id="firstname" value="<?php echo $func->show_str($contact['FIRSTNAME']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
         <p >
            <label for="label"><?php echo _SOCIETY; ?> : </label>
            <input name="society" type="text"  id="society" value="<?php echo $func->show_str($contact['SOCIETY']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/> <?php if(!$readonly){?><span class="red_asterisk" id="society_mandatory" style="display:none;">*</span><?php } ?>
         </p>
          <p id="function_p" style="display:<?php if($contact['IS_CORPORATE_PERSON'] == 'Y'){ echo 'none';}else{ echo 'block';}?>;">
            <label for="label"><?php echo _FUNCTION; ?> : </label>
            <input name="function" type="text"  id="function" value="<?php echo $func->show_str($contact['FUNCTION']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
          <p>
            <label for="label"><?php echo _PHONE; ?> : </label>
            <input name="phone" type="text"  id="phone" value="<?php echo $func->show_str($contact['PHONE']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
         <p>
            <label for="label"><?php echo _MAIL; ?> : </label>
            <input name="mail" type="text" id="mail" value="<?php echo $func->show_str($contact['MAIL']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
         <p >
         ...
         <br/>
          <p>
            ...
            <input name="num" type="text"  id="num" value="<?php echo $func->show_str($contact['ADD_NUM']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
         <p>
            <label for="label"><?php echo _STREET; ?> : </label>
            <input name="street" type="text"  id="street" value="<?php echo $func->show_str($contact['ADD_STREET']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
         <p>
            <label for="label"><?php echo _COMPLEMENT; ?> : </label>
            <input name="add_comp" type="text"  id="add_comp" value="<?php echo $func->show_str($contact['ADD_COMP']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
        <p>
            <label for="label"><?php echo _TOWN; ?> : </label>
            <input name="town" type="text" id="town" value="<?php echo $func->show_str($contact['ADD_TOWN']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
         <p>
            <label for="label"><?php echo _POSTAL_CODE; ?> : </label>
            <input name="cp" type="text" id="cp" value="<?php echo $func->show_str($contact['ADD_CP']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
        <p>
            <label for="label"><?php echo _COUNTRY; ?> : </label>
            <input name="country" type="text"  id="country" value="<?php echo $func->show_str($contact['ADD_COUNTRY']); ?>" <?php if($readonly){ echo 'readonly="readonly"';}?>/>
         </p>
        ...
        <p class="buttons">
        <?php if(!$readonly) { ?>
        <input name="submit" type="submit" value="<?php if($_REQUEST['mode']== 'add') echo _ADD; else if($_REQUEST['mode']== 'up') echo _UPDATE;?>"  class="button" />
        <?php } ?>
            <input name="close" type="button" value="<?php echo _CLOSE;?>"  onclick="self.close();" class="button" />
        </p>
    </form >
</div>
<?php if($contact['IS_CORPORATE_PERSON'] == 'Y')
{
    ?>
    <script type="text/javascript">show_admin_contacts(true);</script>
    <?php
} elseif($contact['IS_CORPORATE_PERSON'] == 'N')
{
    ?>
    <script type="text/javascript">show_admin_contacts(false);</script>
    <?php
}?>
</body>
</html>
