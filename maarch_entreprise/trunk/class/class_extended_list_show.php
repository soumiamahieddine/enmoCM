    ...
    private function create_content($result, $theline, $listcolumn, $key)
    {
        ...
        for($count_column = 0;$count_column < count($listcolumn);$count_column++)
        {
            if($result[$theline][$count_column]['show']==true)
            {
                ...
                {
                    $content .= '<td width="'.$result[$theline][$count_column]['size'].'%" align="'.$result[$theline][$count_column]['align'].'" onclick="valid_form( \'page\', \''.$result[$theline][0]['value'].'\', \''.$this->params['id_action'].'\');" '.$class.'>'.$this->show_str($this->extended_thisword($result[$theline][$count_column]['value'], $this->what)).'</td>';
                }//Open popup on line click
                elseif($this->params['bool_link_on_line'] && $disabled_field === false)
                {
                    $content .= '<td width="'.$result[$theline][$count_column]['size'].'%" align="'.$result[$theline][$count_column]['align'].'" onclick="openpopup(\''.$this->params['link_on_line_destination'].'?id='.$result[$theline][0]['value'].'\');" '.$class.'>'.$this->show_str($this->extended_thisword($result[$theline][$count_column]['value'], $this->what)).'</td>';
                }
                else//Default
                {
                    $content .= '<td width="'.$result[$theline][$count_column]['size'].'%" align="'.$result[$theline][$count_column]['align'].'" '.$class.'>'.$this->show_str($this->extended_thisword($result[$theline][$count_column]['value'], $this->what)).'</td>';
                }
            }
        }
