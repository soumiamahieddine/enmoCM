...
class indexing_searching_app extends dbquery
{
    ...
    public function update_business($post, $typeform, $id_to_update, $coll_id)
    {
        require_once('core/class/class_request.php');
        require_once('core/class/class_history.php');
        require_once('core/class/class_security.php');
        ...
        foreach (array_keys($post) as $key) {
            if ($_ENV['categories'][$cat_id][$key]['modify'] == true) {
                ...
                if (isset($_ENV['categories'][$cat_id][$key]['type_form'])
                    && $_ENV['categories'][$cat_id][$key]['type_form'] == 'integer'
                    && ! empty($post[$key])
                    && preg_match("/^[0-9]*$/", $post[$key]) == 0
                    && (is_int($post[$key]) || is_float($post[$key]))
                ) {
                    //if ($_ENV['categories'][$cat_id][$key]['type_form'] == 'integer'  && preg_match("/^[0-9]+$/",$post[$key])== 0) {
                    $_SESSION['error'] .= $_ENV['categories'][$cat_id][$key]['label']." "._WRONG_FORMAT." <br/>";
                }  else if ($_ENV['categories'][$cat_id][$key]['type_field'] == 'integer' && $_ENV['categories'][$cat_id][$key]['table'] <> 'none' && $post[$key] != '') {
        ...
        if ($core->is_module_loaded('folder')) {
            ...
            if (!empty($project_id) && !empty($market_id)) {
                $request->query("select folders_system_id from " 
                    . $_SESSION['tablename']['fold_folders']." where folders_system_id = ".$market_id." and parent_id = ".$project_id);
                if ($request->nb_result() == 0) {
                    $_SESSION['error'] .= _INCOMPATIBILITY_MARKET_PROJECT.'<br/>';
                }
            }
            ...
            if (empty($_SESSION['error'])) {
                ...
                if ($folder_id <> $old_folder_id && $_SESSION['history']['folderup']) {
                    ...
                    $hist->add($_SESSION['tablename']['fold_folders'], $folder_id, "UP",'folderup',
                        _DOC_NUM.$id_to_update._ADDED_TO_FOLDER, $_SESSION['config']['databasetype'],'apps');
                    if (isset($old_folder_id) && !empty($old_folder_id))
                    {
                        $hist->add($_SESSION['tablename']['fold_folders'], $old_folder_id, "UP", 'folderup', 
                            _DOC_NUM.$id_to_update._DELETED_FROM_FOLDER, $_SESSION['config']['databasetype'],'apps');
                    }
                }
            }
        }
